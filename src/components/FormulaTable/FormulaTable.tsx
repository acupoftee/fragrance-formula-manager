...
const FormulaTable = () => {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (category) {
      filteredItems = filteredItems.filter(
        (formula: Formula) => formula.category.toLowerCase() === category
      );
    }
    if (filter) {
      ...
      filteredItems = filteredItems.filter(
        (formula: Formula) =>
          formula.name.toLowerCase().includes(lowercaseFilter) ||
          formula.notes.toLowerCase().includes(lowercaseFilter)
      );
    }
  ...
  const handleSelectedRows = useCallback(
    ({ selectedRows }: { selectedRows: Formula[] }) =>
  ...
  const subHeaderComponentMemo = useMemo(() => {
    ...
    return (
      <Box flexDirection="row" width="100%">
        ...
        {(filter || category) && (
          <Link
            sx={{ m: 1, cursor: "pointer", textDecoration: "none" }}
            onClick={handleClear}
          >
            Clear
          </Link>
